...
var Promise = require( 'promise' ),
    ...
    execPlugins = require( './execplugins' ),

    rFirstStr = /[\s\r\n\=]/,
    rDefine = /define\(\s*(['"](.+?)['"]\s*,)?(\s*\[[^\]]*\]\s*,)?/,
    rDeps = /(['"])(.+?)\1/g,
    rAlias = /alias\s*\:([^\}]+)\}/,
    rPaths = /paths\s*\:([^\}]+)\}/,
    rVars = /vars\s*\:([^\}]+)\}/,
 ...
 */
var filterIgnore = function( ignore, id, origId, options ){
        ...
        return ignore.some(function( item ){
            ...
            // ui 将匹配 ../ui 和 ../../ui
                ...
                return parseConfigParam(options, item) === id;
            }
        });
    },
    ...
    parsePaths = function (config, origId) {
        var arr, modId;
        if( config.paths ){
            arr = origId.split( '/' );
    ...
    // 通过seajs配置解析路径
    parseConfigParam = function (options, origId) {
        if( options.map && options.map[origId] ){
            origId = options.map[ origId ];
        }
        // 处理seajs.config => vars
        if( options.config.vars ){
            if( ~origId.indexOf('{') ){
                origId = origId.replace( rVar, function( $, $1 ){
                    if( options.config.vars[$1] ){
                        return options.config.vars[$1];
                    }
        ...
        if( options.config.alias && options.config.alias[origId] ){
            origId = options.config.alias[ origId ];
        }

        // 处理seajs.config => paths
        origId = parsePaths(options.config, origId);
        return origId
    },
    /*
    ...
    mergePath = function( options, deps, base ){
        ...
        return deps.map(function( item, i ){
            ...
            origId = parseConfigParam(options, origId);
            
            if (config.alias && config.alias[item.id]) {
                id = origId;
            } else {
                id = origId;
            }
            ...
            log('base', base);
            log('origId', origId);
            log('resolve', path.resolve( base, origId ));
            log('*********');
            return {
                id : id,
                extName : item.extName,
                path : path.resolve( base, origId.replace(/^\//, '')),
                origId : origId
            };
        });
    },
    ...
    modPathResolve = function( options, filePath ){
        ...
        if (extName !== '.css' && extName !== '.html' && extName !== '.tpl' && extName !== '.cshtml') {
            extName = '.js';
        }
        if( extName && extName === '.js' ){
            id = id.replace( extName, '' );
        }
        filePath = filePath.replace('text!', '');
        return {
            id : id,
            path : filePath,
            extName : extName
        ...
        var promiseArr = parentDeps.map(function( item ){
            return new Promise(function( resolve, reject ){
                ...
                isIgnore = options.ignore ?
                    filterIgnore( options.ignore, id, origId, options ) :
                    false;
                ...
                // 处理普通的js模块
                    ...
                    if( (!extName || extName === '.js') && filePath.slice(-3) !== '.js' ){
                        filePath += '.js'
                    }
    ...
    pullDeps = function( options, reg, contents ){
        ...
        while( (matches = reg.exec(contents)) !== null ){
            ...
            if( origId && origId.slice(0, 4) !== 'http' ){
                var depPathResult = modPathResolve( options, origId );
        ...
        if( !isSeajsUse ){
            contents = contents.replace( rRequire, function( $, _, $2 ){
                ...
                if( $2 && $2.slice(0, 4) !== 'http' ){
                    ...
                    depOrigId = parsePaths(options.config, depOrigId);
            ...
            contents = contents.replace( rDefine, function(match, idStr, modId){
                var id = modId ? parseConfigParam(options, modId) : idMap[ origId ];
                return deps.length ?
                    "define('" + id + "',['" + deps.join("','") + "']," :
                    "define('" + id + "',";
            });
    ...
    // 解析传入的config文件
    parseConfigFile = function (contents) {
    ...
    createStream = function( options ){
        ...
        if( options ){
            ...
            if (options.configFile && options.configFile != 'undefined'){
                configFile = fs.readFileSync(options.configFile, 'UTF-8');
                configFile = parseConfigFile(configFile);
                o.config = configFile.config;
            }
        }
        ...
        return through.obj(function( file, enc, callback ){
            if( file.isBuffer() ){
                ...
                o.contents = '';
                if (o.base) {
                    o.baseUrl = path.resolve(file.path, '../');
                    // o.baseUrl = path.resolve(o.baseUrl, o.base);
                    var arr;
                    if (o.baseUrl.indexOf('/') !== -1) {
                        arr = o.baseUrl.split('/');
                    } else {
                        arr = o.baseUrl.split('\\');
                    }
